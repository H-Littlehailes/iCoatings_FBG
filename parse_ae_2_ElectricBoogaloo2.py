...
def butter_bandpass_filter(data, lowcut, highcut, order=6):
    sos = butter_bandpass(lowcut, highcut, 1e6)#, order=6)
...
def plotSTFTPLT(signal,nperseg=256,fclip=None,use_log=True,**kwargs):
    ...
    if use_log:
        X = ax.pcolormesh(t, f, np.abs(Zxx), norm=colors.LogNorm(vmin=np.abs(Zxx).max()*0.01, vmax=np.abs(Zxx).max()),cmap=kwargs.get("cmap",'jet'),shading=kwargs.get("shading",'gouraud'))
    else:
        X = ax.pcolormesh(t, f, np.abs(Zxx),cmap=kwargs.get("cmap",'jet'),shading=kwargs.get("shading",'gouraud'))
    # set title
    ...
    for fn in glob(path):
        ...
        if periods:
            ...
            for sname,chunk in periods.items():
                ...
                f0=plotSTFTPLT(i0,nperseg,fclip,use_log,title=f"{os.path.splitext(os.path.basename(fn))[0]}, Input 0, {sname} STFT",tmin=chunk[0],**kwargs)
                if fclip:
                    f0.savefig(fr"{os.path.splitext(fn)[0]}-Input 0-stft-stripe-{sname}-clip-{fclip}.png")
                else:
                    f0.savefig(fr"{os.path.splitext(fn)[0]}-Input 0-stft-stripe-{sname}.png")
                #plt.close(f)
                #return f0

                # perform stft for each channel
                f1=plotSTFTPLT(i1,nperseg,fclip,use_log,title=f"{os.path.splitext(os.path.basename(fn))[0]}, Input 1, {sname} STFT",tmin=chunk[0],**kwargs)
                
                if fclip:
                    f1.savefig(fr"{os.path.splitext(fn)[0]}-Input 1-stft-stripe-{sname}-clip-{fclip}.png")
                else:
                    f1.savefig(fr"{os.path.splitext(fn)[0]}-Input 1-stft-stripe-{sname}.png")
                #plt.close(f)
            return f0, f1
        else:
...
def bandpassfilterStripes(fn,freq = [150e3, 160e3],mode=['highpass','lowpass'],**kwargs):  #(fn,freq=50e3,mode='highpass',**kwargs)
    '''
        Apply filters to each identified stripe in the file
    ...
    if periods:
        ...
        for sname,chunk in periods.items():
            ...
            sos = butter(kwargs.get("order",10), 150e3, 'highpass', fs=1e6, output='sos',analog=False)
            i0 = sosfilt(sos, i0)
            i1 = sosfilt(sos, i1)
            sos = butter(kwargs.get("order",10), 160e3, 'lowpass', fs=1e6, output='sos',analog=False)
            i0 = sosfilt(sos, i0)
            i1 = sosfilt(sos, i1)
               # if it's a list/tuple then it's a bandpass filter
             #  elif isinstance(c,(tuple,list)):
           ...
           #plot data
            ax_i0.plot(time,i0,'r-',label="Filtered")
            ax_i0.legend()
            ax_i0.set(xlabel="Time (s)",ylabel="Voltage (V)")#,title=f"{fname}, Input 0, {sname}, {modes_plot_string}, {freq_plot_string}Hz", fontsize=24)
            ax_i0.set_title(f"{fname}, Input 0, {sname}, {modes_plot_string}, {freq_plot_string}Hz", fontsize=9)
            #f_i0.savefig(fr"{os.path.splitext(fn)[0]}-Input 0-time-stripe-{sname}-filtered-{modes_save_string}-freq-{freq_save_string}.png")
            #plt.close(f_i0)
            #plot data
            ...
            ax_i1.legend()
            ax_i1.set(xlabel="Time (s)",ylabel="Voltage (V)")#,title=f"{fname}, Input 1, {sname}, {modes_plot_string}, {freq_plot_string}Hz")
            ax_i1.set_title(f"{fname}, Input 1, {sname}, {modes_plot_string}, {freq_plot_string}Hz", fontsize=9)
            #f_i1.savefig(fr"{os.path.splitext(fn)[0]}-Input 1-time-stripe-{sname}-filtered-{modes_save_string}-freq-{freq_save_string}.png")
            #plt.close(f_i1)
...
#    STFTWithLombScargle("ae/sheff_lsbu_stripe_coating_1.tdms",span=0.4,n_bins=100, grid_size=100,min_freq=300e3,max_freq=400e3)
 ...
 #   filterStripes("ae/sheff_lsbu_stripe_coating_3_pulsing.tdms",300e3,'highpass')
    #plotSignalEnergyFreqBands("ae/sheff_lsbu_stripe_coating_2.tdms",50e3,tclip=[107.5,109],fmin=100e3)
...
##    periodogramStripes("ae/sheff_lsbu_stripe_coating_2.tdms")
  ##  welchStripesOverlap("ae/sheff_lsbu_stripe_coating_2.tdms",freq_clip=[00e3,50e3],use_fr=True)
    #welchStripes("ae/sheff_lsbu_stripe_coating_2.tdms",freq_clip=[0,50e3],use_fr=True)
  ##  drawEdgeAroundStripe("ae/sheff_lsbu_stripe_coating_2.tdms",dist=int(50e3),mode="separate", stripe_ref = "stripe_1_4")
  
    plotSTFTStripes("ae/sheff_lsbu_stripe_coating_2.tdms",nperseg=5120,fclip=None,use_log=True)
...
##    filterStripes("ae/sheff_lsbu_stripe_coating_1.tdms",50e3,mode="lowpass")
   ...
   # bandpassfilterStripes("ae/sheff_lsbu_stripe_coating_2.tdms",freq = [150e3, 160e3])
...
##    stripeSpectrogram("ae/sheff_lsbu_stripe_coating_1.tdms")
   ...
   # stripeSpecgram("ae/sheff_lsbu_stripe_coating_2.tdms", shift=False)
