# -*- coding: utf-8 -*-
"""
Created on Tue Jun  4 13:07:47 2024
...
file = "G:/My Drive/LSBU_Trip_Mar2024/Plate1_Data.txt"
...
time_T1, T1 = TemperatureProbeCalc(file, t_index = 7, index = 9, start = 44, end = 961)
time_T2, T2 = TemperatureProbeCalc(file, t_index = 7, index = 12, start = 962, end = 1777)
time_T3, T3 = TemperatureProbeCalc(file, t_index = 7, index = 8, start = 1778, end = 2506)
time_T5, T5 = TemperatureProbeCalc(file, t_index = 7, index = 11, start = 3365, end = 4295)
...
time_T3 = np.array([time_to_seconds(t) for t in time_T3])
time_T5 = np.array([time_to_seconds(t) for t in time_T5])

time_St1, St1 = TemperatureProbeCalc(file, t_index = 0, index = 2, start = 44, end = 961)
time_St2, St2 = TemperatureProbeCalc(file, t_index = 0, index = 5, start = 962, end = 1777)
time_St3, St3 = TemperatureProbeCalc(file, t_index = 0, index = 1, start = 1778, end = 2506)
time_St5, St5 = TemperatureProbeCalc(file, t_index = 0, index = 4, start = 3365, end = 4295)

time_St1 = np.array([(time_to_seconds_long(t)-10) for t in time_St1])
time_St2 = np.array([(time_to_seconds_long(t)-10) for t in time_St2])
time_St3 = np.array([(time_to_seconds_long(t)-10) for t in time_St3])
...
time_1 = np.linspace(0, 918, 918)
time_2 = np.linspace(0, 816, 816)
time_3 = np.linspace(0, 729, 729)
...
def DTG_Strain(Stripe_Strain, Stripe_Temp, Time, w0_index, compensation = True, coating = 'ORMOCER'):
    ...
    ax1.plot(time_3, (result_columns)-result_columns[0], label=f"T. Compensated Strain Stripe 3 (50 mbar)")
    ax1.legend()
    ax1.grid()
    ...
    fig1, ax0 = plt.subplots(1,1,constrained_layout=True)
    ax1 = ax0.twinx()
    ax1.plot(time_T3, Stripe_Temp, 'red')
    ax0.plot(time_St3, Stripe_Strain, 'blue' )
    ax0.set_xlabel("Time (s)")
    ...
    #Output file name
    filename = 'Substrate1_Stripe3_CompensatedStrain_Origin.txt'
        
    # Export lists to text file
    #export_lists_to_text(time_5, result_columns, filename)
...
DTG_Strain( St3, T3, time_T3, w0_index = 0,  compensation = True, coating = 'ORMOCER')
