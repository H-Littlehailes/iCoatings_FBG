...
def plotLombScargle(path, normalize=True,**kwargs): #freqs=np.arange(100e3,350e3,1e3),
    '''
    ...
    for fn in glob(path):
        ...
        if periods:
            ...
            freqs=np.arange(100e3,350e3,5000000)
            rads=[]
            for i in freqs:
                rads = freqs/(2*np.pi)
            rads = np.append(rads, rads)
            ...
            for sname,chunk in periods.items():
                ...
                #data = loadTDMSData(path)
                    ...
                    # calculate
                pgram = lombscargle(time, i0, rads, normalize=normalize)
                ...
                pgram = lombscargle(time, i1, rads, normalize=normalize)
    ...
    for fn in glob(path):
        ...
        if periods:
            ...
            for sname,chunk in periods.items():
                ...
                time,i0,i1 = loadTDMSSub(path,chunk,is_time=True)
                # plot period
                f, Pxx_i0 = signal.welch(i0, 1e6,nperseg=1024) #1024
                if kwargs.get("freq_clip",None):
                    f0,f1 = kwargs.get("freq_clip",None)
...
if __name__ == "__main__":
  ...
  ##  replotAE("ae/sheff_lsbu_stripe_coating_2.tdms",clip=True,ds=100)
   ...
   # plotLombScargle("ae/sheff_lsbu_stripe_coating_2.tdms",freqs=np.arange(100e3,350e3,1e3),normalize=True)
    #plotLombScargle("ae/sheff_lsbu_stripe_coating_3_pulsing.tdms",normalize=True)  #freqs=np.arange(100e3,350e3,1e3),
...
##    filterStripesProportion("ae/sheff_lsbu_stripe_coating_2.tdms",minfreq=10e3)
    ...
    ## periodogramStripes
##    welchStripesOverlap("ae/sheff_lsbu_stripe_coating_1.tdms",freq_clip=[29e3,35e3],use_fr=True)
...
##    stripeSpectrogram("ae/sheff_lsbu_stripe_coating_3_pulsing.tdms")
 ...
 ##   plotStripesLimits("ae/sheff_lsbu_stripe_coating_1.tdms",cutoff_freq=[30e3,300e3],mode=["lowpass","highpass"],plot_mode="both")
    #plotStripesLimits("ae/sheff_lsbu_stripe_coating_2.tdms",cutoff_freq=[50e3],mode=["highpass"],plot_mode="both")
