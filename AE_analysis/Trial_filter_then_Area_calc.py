...
STRIPE_PERIOD_2 = {'sheff_lsbu_stripe_coating_2': {#'ref':  [281, 281.5],#[101,106], 
                                                   #'stripe_1_1': [108.430, 109.083],#[108.430, 109.083], #[107.800, 109.060], 
                                                   ...
                                                   #'stripe_3_1':[127, 132],
                                                   #'stripe_4_1':[137, 142],
                                                   #'stripe_5_1':[146.5, 151.5],
                                                   # 2nd pass
                                                   'stripe_1_2':[157.5, 162.5], 
                                                   'stripe_2_2':[169.5, 174.5],
                                                   'stripe_3_2':[180.5, 185.5],
                                                   'stripe_4_2':[190, 195],
                                                   'stripe_5_2':[199.5, 204.5],
                                                   # 3rd pass
                                                   'stripe_1_3':[215.5, 220.5],
                                                   'stripe_2_3':[226, 231], 
                                                   'stripe_3_3':[237.5, 242.5],
                                                   'stripe_4_3':[249.5, 254.5],
                                                   'stripe_5_3':[260.5, 265.5],
                                                   # unknown
                                                   'stripe_1_4':[277, 282],
                                                   'stripe_2_4':[290.5, 295.5],
                                                   'stripe_3_4':[304.5, 309.5],
                                                   'stripe_4_4':[314.5, 319.5],
                                                   'stripe_5_4':[327.5,332.5],
                                                   }}
...
def plotSTFTPLT(signal,nperseg=256,fclip=None,use_log=True,**kwargs):
    ...
    #print(len(xrec))
    #butter_bandpass_filter(Zxx, 1, 100e3)
    max_value = np.max(np.abs(Zxx))
    min_value = np.min(np.abs(Zxx))
    #Zxx = Zxx/max_value
    Zxx = (Zxx - min_value)/(max_value - min_value)
    #Zxx = (Zxx>=0.5) & (Zxx<=1)
    #X_raw = [t, f, Zxx]
...
#    fmt ='% s')
    ...
    if use_log:
        ...
        X = ax.pcolormesh(t, f, np.abs(Zxx), norm=colors.LogNorm(vmin=1e-4, vmax=1),cmap=kwargs.get("cmap",'inferno'),shading=kwargs.get("shading",'gouraud'))
...
def find_local_intensity_points(frequency, time, intensity, threshold=[0.15,0.16]):
    """
    Find local points of intensity in an STFT spectrogram.

    Args:
    - frequency: 1D array representing frequency values
    - time: 1D array representing time values
    - intensity: 2D array representing intensity values (frequency x time)
    - threshold: Threshold value for intensity

    Returns:
    - intensity_points: List of tuples (frequency_index, time_index) representing local intensity points
    """
    # Normalize the intensity
    intensity /= np.max(intensity)

    # Find local peaks using scipy's find_peaks function
    peaks, _ = signal.find_peaks(intensity.ravel(), height=threshold)

    # Convert peak indices to frequency and time indices
    freq_indices, time_indices = np.unravel_index(peaks, intensity.shape)

    # Extract frequency and time values at peak indices
    peak_freqs = frequency[freq_indices]
    peak_times = time[time_indices]

    # Combine frequency and time indices into tuples
    intensity_points = list(zip(peak_freqs, peak_times))
    
    fig,ax = plt.subplots(constrained_layout=True)
    plt.scatter(peak_times, peak_freqs)
    #print(intensity_points)
    #np.savetxt(f"STFT_Peak_Finder_5.csv",   #Commented out 20240119
    #    intensity_points, delimiter =", ",  # Set the delimiter as a comma followed by a space
    #    fmt ='% s')

def STFT_Peak_Finder(path, nperseg=2560, fclip=None,**kwargs):
    for fn in glob(path):
        periods = list(filter(lambda x : os.path.splitext(os.path.basename(fn))[0] in x,PERIODS))
        if periods:
            ...
            for sname,chunk in periods.items():
                ...
                if fclip:
                    ...
                    Zxx = Zxx[(f>=fclip[0]) & (f<=fclip[1])]
                    f = f[(f>=fclip[0]) & (f<=fclip[1])]
                intensity_points = find_local_intensity_points(f, t, abs(Zxx))
...
def plotSTFTPLT_ML(signal,nperseg=256,fclip=None,use_log=True,**kwargs):
    '''
        Plot the STFT of the target signal using Matplotlib
...
def plotSTFTStripes_ML(path,nperseg=2560,fclip=True,use_log=True,**kwargs):
    '''
        Plot the STFT of each stripe in each channel
    ...
    for fn in glob(path):
        ...
        if periods:
            ...
            for sname,chunk in periods.items():
                time,i0,i1 = loadTDMSSub(path,chunk,is_time=True)
                # perform stft for each channel
                f1=plotSTFTPLT_ML(i0,nperseg,fclip,use_log,title=f"{os.path.splitext(os.path.basename(fn))[0]}, Input 0, {sname} STFT",tmin=chunk[0],**kwargs)
                if fclip:
                    f1.savefig(fr"{os.path.splitext(fn)[0]}-Input 0-stft-stripe-{sname}-clip-{fclip}.png")
                else:
                    f1.savefig(fr"{os.path.splitext(fn)[0]}-Input 0-stft-stripe-{sname}.png")
    ...
    def process_images(input_folder, output_folder, subsection_size):
        for filename in os.listdir(input_folder):
            if filename.endswith(",png"):
                ...
                export_subsections(subsections, output_folder, os.path.splittext(filename)[0])
    if __name__ == "__main__":
        input_folder = "input_images" #folder containing original png images
    ...
    for fn in glob(path):
        ...
        if periods:
            ...
            for sname,chunk in periods.items():
                ...
                # Generate synthetic voltage-time data
                fs = 1e6  # Sampling frequency
                #x = chirp(time, i0)

                # Perform continuous wavelet transform
                wavelet = 'cmor'  # Continuous wavelet to use
                scales = np.arange(1, 56)  # Range of scales to use
                coefficients, frequencies = pywt.cwt(i0, scales, wavelet, sampling_period=1/1e6)
    
                # Plot the results
                plt.figure(figsize=(10, 6))
                plt.imshow(np.abs(coefficients), extent=[0, len(time), frequencies[-1], frequencies[0]], aspect='auto', norm=colors.LogNorm(vmin=np.abs(coefficients).min(), vmax=np.abs(coefficients).max()), cmap=kwargs.get("cmap",'cool'))
                plt.colorbar(label='Magnitude')
                plt.xlabel('Time')
                plt.ylabel('Frequency (Hz)')
                plt.title(f"{os.path.splitext(os.path.basename(fn))[0]}, Input 0, {sname} STFT")
                plt.show()
...
if __name__ == "__main__":
   ...
   # plotSTFTStripes("ae/sheff_lsbu_stripe_coating_2.tdms", fclip=[280e3,370e3], use_log=True)
    ...
    #STFT_Peak_Finder("ae/sheff_lsbu_stripe_coating_1.tdms", fclip=[100e3,275e3])
    #plotSTFTStripes_ML("ae/sheff_lsbu_stripe_coating_1.tdms", fclip=[100e3,500e3] , use_log=True) #[100e3,500e3]
    #cwt("ae/sheff_lsbu_stripe_coating_2.tdms")
